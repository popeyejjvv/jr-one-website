...
export { isCompanyDomainEmail, companyDomainRejection };
...
export async function runRoiCapture({
  ...
  // Same rule and same reason as the homepage box: BuilderPrime refuses its own
  // company domain with a plain-text 500 that reads as an outage. Guarding here
  // as well as in the route means no caller can skip it. See lib/company-domain.js.
  if (isCompanyDomainEmail(email)) {
    logCompanyDomainRejection({ surface: "roi-calculator-capture", email, logger });
