...
export async function triggerOutboundCall({
  ...
  // ── Phone-only test-number guard (last line of defense) ──
  // Catches test/non-dialable numbers even if the caller forgot to check.
  const phoneDigits = String(phone).replace(/\D/g, "");
  const phoneTen = phoneDigits.length === 11 && phoneDigits.startsWith("1")
    ? phoneDigits.slice(1) : phoneDigits;
  const masked = phoneTen.length >= 4 ? "***" + phoneTen.slice(-4) : "(empty)";

  if (phoneTen.length < 10) {
    console.warn(`VAPI-GUARD: suppressed call to non-dialable number ${masked} (fewer than 10 digits)`);
    return { skipped: true, reason: "non-dialable" };
  }
  if (/^(\d)\1{9}$/.test(phoneTen)) {
    console.warn(`VAPI-GUARD: suppressed call to test number ${masked} (all same digit)`);
    return { skipped: true, reason: "test-number" };
  }
  const phoneExchange = phoneTen.slice(3, 6);
  if (phoneExchange === "555") {
    console.warn(`VAPI-GUARD: suppressed call to test number ${masked} (555 exchange)`);
    return { skipped: true, reason: "test-number" };
  }
  if (phoneTen === "1234567890") {
    console.warn(`VAPI-GUARD: suppressed call to test number ${masked} (sequential dialpad)`);
    return { skipped: true, reason: "test-number" };
  }
  if (phoneTen === "8444443114") {
    console.warn(`VAPI-GUARD: suppressed call to own line ${masked} (JR One number)`);
    return { skipped: true, reason: "test-number" };
  }
