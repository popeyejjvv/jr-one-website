...
// Module-level cache of the empty fallback so pages render cleanly when CompanyCam is down.
let _emptyCacheBuilt = null;
function emptyGallery() {
  if (_emptyCacheBuilt) return _emptyCacheBuilt;
  const byCity = {};
  for (const slug of ALL_CITY_SLUGS) {
    byCity[slug] = { citySlug: slug, photos: [], totalProjects: 0 };
  }
  byCity["__out_of_area__"] = { citySlug: "__out_of_area__", photos: [], totalProjects: 0 };
  _emptyCacheBuilt = byCity;
  return byCity;
}

// Internal: fetch the full gallery from /api/companycam, group by city slug, cache for ISR.
// Graceful degradation: when CompanyCam returns non-2xx (502 rate-limit, 500 outage, etc.)
// we return an empty gallery so the page renders without a portfolio rather than failing
// the build. Lighthouse and the quality gate still catch missing-portfolio cases via
// companycam-presence check (Phase 2).
async function loadGalleryByCity() {
  let res;
  try {
    res = await fetch(GALLERY_URL, { next: { revalidate: 3600, tags: ["companycam-gallery"] } });
  } catch (err) {
    console.warn(`[companycam] fetch threw: ${err?.message || err} — rendering empty gallery`);
    return emptyGallery();
  }
  if (!res.ok) {
    console.warn(`[companycam] gallery fetch failed status=${res.status} — rendering empty gallery`);
    return emptyGallery();
  }
  let payload;
  try {
    ...
    console.warn(`[companycam] gallery JSON parse failed: ${err?.message || err} — rendering empty gallery`);
    return emptyGallery();
  }
