...
import { interleaveByProject } from "./photo-select";
...
function finalizeByCity(byCity) {
  for (const slug of Object.keys(byCity)) {
    ...
    // Round-robin across projects (newest project first) instead of a flat newest-first
    // sort, so a city page never shows consecutive tiles from the same job.
    byCity[slug].photos = interleaveByProject(byCity[slug].photos);
...
async function loadGalleryByCity() {
  ...
  let res;
  try {
    // Hard 8s timeout: a slow gallery endpoint must degrade to the inventory
    // fallback, never eat the 60s static-export budget of the page awaiting it
    // (456 /areas pages died exactly that way on 2026-07-15).
    res = await fetch(GALLERY_URL, {
      next: { revalidate: 3600, tags: ["companycam-gallery"] },
...
export async function getCountyFallbackJobs(county, opts = {}) {
  ...
  const interleaved = interleaveByProject(pool);
  return interleaved.slice(0, limit).map((p) => ({ ...p, capturedAt: isoDate(p.capturedAt) }));
}
